# outbound-studio-working-spec-terminated-thread-what-should-have-shipped
# Outbound Studio — Working Spec (terminated thread, what should have shipped)

The actual deliverable the user asked for in their first message: skills, config, process, crew orchestration with scope/inputs/outputs defined per agent. Replaces all prior noise. Drop this into the next thread to continue.
...
Each step in the flow maps to a specific connected skill or tool. Nothing custom needs to be built that the stack doesn't already cover.
...
| Knowledge stack / component grading | **Supabase** (component_library + vertical_pack tables) |

Everything in this row labeled 'should have been verified turn 1' was the user's instruction in their first message and was skipped.
...
## Pricing reference (the useful research finding)
